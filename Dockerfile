...
ENV BITCOIN_ASC_URL https://bitcoincore.org/bin/brocoin-core-$BITCOIN_VERSION/SHA256SUMS.asc
...
RUN [ -n "$STANDALONE" ] || \
    ...
    && wget -qO brocoin.asc "$BITCOIN_ASC_URL" \
    && gpg --verify brocoin.asc \
    && cat brocoin.asc | grep "$BITCOIN_FILENAME" | sha256sum -c - \
    && BD=brocoin-$BITCOIN_VERSION/bin \
    && tar -xzvf "$BITCOIN_FILENAME" $BD/bitcoind $BD/brocoin-cli --strip-components=1 \
